...
int main() {
	...
	int okPackageContinue = (lcIspPackageChar == 'a' || lcIspPackageChar == 'b' || lcIspPackageChar == 'c') ? 1 : main(); 
	...
	switch (lcIspPackageChar) {
		case 'a' : packageACost = (packageAMonthly * 12) + ((monthTotalHours - (packageATotal * 12)) * packageAAdditional);
		case 'b' : packageBCost = (packageBMonthly * 12) + ((monthTotalHours - (packageBTotal * 12)) * packageAAdditional);
		case 'c' : packageCCost = (packageCMonthly * 12); break;
		default : break;
	}
